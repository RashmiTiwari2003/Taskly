...
import DeleteUserConfirm from '@/app/(component)/DeleteUserConfirm';
...
const UsersBusiness = () => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            const response = await fetch(`${SERVER}/usersbusiness/${email}`);
            const data = await response.json();
            ...
            <table className='border-collapse w-full'>
                <thead className='top-0 sticky bg-sky-100 font-bold text-left text-sm md:text-base'>
                    <tr>
                        ...
                        <th className={`border-slate-700 p-3 border ${role === "User" ? "hidden" : ''}`}>Actions</th>
                    </tr>
                </thead>
                <tbody className='text-xs md:text-base'>
                    {users.map(user => (
                        <tr key={user.email}>
                            ...
                            <td className={`border-slate-700 px-1 py-3 border text-center ${role === "User" ? "hidden" : ''}`}>
